...
function ref(val) {
  ...
  //1.区分响应式原始类型,在wrapper对象添加不可枚举属性,值为value
...
function proxyRefs(target) {
  return new Proxy(target, {
    get(target, key, receiver) {
      ...
      // 如果获取的对象有__v_isRef ,返回他的value值
      return value.__v_isRef ? value.value : value
    },
    set(target, key, newVal, receiver) {
      let value = target[key]
      //如果是 toRef 代理的数据,对它的value进行修改
      if (value.__v_isRef) {
        value.value = newVal
        return true
      }
...
effect(() => {
  // 渲染一个render
  render(`<div>hello ${count.value}</div>`, document.getElementById('app'))
})
